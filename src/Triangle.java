...
/**
 * This is a basic shape that draws an isosceles triangle using the Path2D.double class.
 * It accepts x and y values that specify the upper left corner of the shape
 * along with the dimension for the triangle's base, height, and the thickness of its border.
...
public class Triangle extends DrawingObject {
    ...
    /**
     * This creates a Triangle object with the parameters specified.
     ...
     */
    public Triangle(double x, double y, double height, double base, float borderThickness, Color fillColor, Color borderColor) {
        this.x = x;
        this.y = y;
        this.height = height;
        this.base = base;
    ...
    public void draw(Graphics2D g2d) {
        ...
        // generates the base
        triangle.lineTo(x + base, y + height);

        // generates the side lengths
        triangle.lineTo(x + (base / 2), y);
        triangle.closePath();
